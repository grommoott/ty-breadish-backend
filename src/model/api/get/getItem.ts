...
export default async function getItem(id: ItemId): Promise<IProduct | IRecipe | Error> {
    try {
        ...
        if (productWithId.rowCount == 1) {
            queryRowToProduct(productWithId.rows[0])

        }

        const recipeWithId: QueryResult = await bdClient.query(`select * from recipe where item_id=${id}`)

        if (recipeWithId.rowCount == 1) {
            queryRowToRecipe(recipeWithId.rows[0])
        }
